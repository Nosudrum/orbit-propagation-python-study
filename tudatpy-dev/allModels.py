...
from tudatpy.kernel.numerical_simulation.propagation_setup.integrator import CoefficientSets, OrderToIntegrate
...
print(f"Tudatpy propagation started with following settings :")
...
simulation_end_epoch = time_conversion.julian_day_to_seconds_since_epoch(
    time_conversion.calendar_date_to_julian_day(dates["end_date"] - dates["step_size"]))

# Create default body settings and bodies system
print("Creating bodies system...")
...
if propagation_name == "keplerian":
    ...
    acceleration_settings_spacecraft = dict(
        ...
        Earth=[
            propagation_setup.acceleration.spherical_harmonic_gravity(15, 15),
...
print("Setting dependent variables to be saved...")
...
print("Creating termination settings...")
...
print("Creating propagation settings...")
...
print("Creating numerical integrator settings...")
...
integrator_settings = propagation_setup.integrator.runge_kutta_fixed_step_size(
    simulation_start_epoch, fixed_step_size, CoefficientSets.rkdp_87, order_to_use=OrderToIntegrate.lower
)
...
print("Extracting states and dependent variables...")
...
keplerian_states[:, 2:6] = keplerian_states[:, 2:6] * 180 / np.pi
ecef_states = dependent_variables_history_array[:, 13:19]

# Compute shadow function
print("Computing shadow function...")
